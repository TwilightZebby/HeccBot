...
module.exports = {
    ...
    async execute(selectInteraction)
    {
        const AddRoleSelect = new ActionRowBuilder().addComponents([
            new RoleSelectMenuBuilder().setCustomId(`configure-menu-add-role`).setMinValues(1).setMaxValues(1).setPlaceholder(localize(selectInteraction.locale, 'ROLE_MENU_ROLE_ADD_SEARCH'))
        ]);
        
        const RemoveRoleSelect = new ActionRowBuilder().addComponents([
            new RoleSelectMenuBuilder().setCustomId(`configure-menu-remove-role`).setMinValues(1).setMaxValues(1).setPlaceholder(localize(selectInteraction.locale, 'ROLE_MENU_ROLE_REMOVE_SEARCH'))
        ]);
        
        const SetTypeSelect = new ActionRowBuilder().addComponents([
            new StringSelectMenuBuilder().setCustomId(`configure-set-menu-type`).setMinValues(1).setMaxValues(1).setPlaceholder(localize(selectInteraction.locale, 'ROLE_MENU_SELECT_MENU_TYPE')).setOptions([
        ...
        switch (SelectedOption)
        {
            ...
            case "configure-embed":
                ...
                let embedModal = new ModalBuilder().setCustomId(`configure-menu-embed`).setTitle(localize(selectInteraction.locale, 'ROLE_MENU_CONFIGURE_MENU_EMBED')).addComponents([
        ...
        .then(async fetchedMessage => {
            ...
            .then(async sentMessage => {
                ...
                fs.writeFile('./JsonFiles/Hidden/RoleMenus.json', JSON.stringify(RoleMenuJson, null, 4), async (err) => {
                    if ( err )
                    { 
                        await selectInteraction.followUp({ content: localize(selectInteraction.locale, 'ROLE_MENU_ERROR_CONFIGURATION_GENERIC'), ephemeral: true });
                        return;
                    }
                });
